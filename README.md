...
- 「提取信息」窗口可同时提取带符号的指标参数条款：默认识别 `★ # △ ▲`（可在「自定义符号」中增删或输入其他符号），正文段落和表格整行完整提取，Word 自动编号重建为带序号文本，输出 `数量序号/符号/详细内容（带序号）` 三列的 `_指标参数.xlsx`
...
窗口中还提供「同时提取带符号的指标参数」选项，用于抽取采购需求中带 `★`、`#`、`△`、`▲` 等标记的条款（默认这 4 个符号，点「自定义符号…」可增删或输入其他符号；支持正文与表格、纯文本编号与 Word 自动编号），每个 Word 生成一个 `原文件名_指标参数.xlsx`。详见使用教程第七章。
